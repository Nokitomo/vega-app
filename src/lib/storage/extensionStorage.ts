...
export class ExtensionStorage {
  ...
  /**
   * Add or update a provider source.
   */
  ...
  /**
   * Get the default provider source.
   */
  ...
  /**
   * Get available providers for a source.
   */
  ...
  /**
   * Set available providers for a source.
   */
  setAvailableProviders(
    authorOrProviders: string | ProviderExtension[],
    providers?: ProviderExtension[],
  ): void {
    if (Array.isArray(authorOrProviders)) {
    ...
    mainStorage.setArray(
      this.scopedKey(ExtensionKeys.AVAILABLE_PROVIDERS, authorOrProviders),
      providers || [],
    );
  }
  ...
  getProvidersNeedingUpdate(author?: string): ProviderExtension[] {
    const activeAuthor = author || this.getProviderSource()?.author || '';
    const installed = activeAuthor
      ? this.getInstalledProviders().filter(
          p => p.source?.author === activeAuthor,
        )
      : this.getInstalledProviders();
    const available = this.getAvailableProviders(activeAuthor);
